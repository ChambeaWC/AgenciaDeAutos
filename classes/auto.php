...
class Auto
{
	...
	public function __construct(int $id, string $marca, string $modelo, int $anio, float $precio)
	{
		$this->id = $id;
		$this->setMarca($marca);
	...
	{
		if (!self::esAnioValido($anio)) {
			throw new InvalidArgumentException('El anio no es valido.');
		}
		$this->anio = $anio;
	}
